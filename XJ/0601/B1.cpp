...
class chain{
	...
	edge *&front(){
	    if(!tag)return get()->head;
	    else return get()->tail;
	}
	edge *&back(){
	    if(!tag)return get()->tail; 
	    else return get()->head; 
	}
	void pop_front(){
	    if(this!=p){get()->pop_front();return;}
	    if(!tag)head=head->nxt();
	    else tail=tail->pre();
	    if(head==NULL||tail==NULL)head=NULL,tail=NULL;
	}
	void pop_back(){
	    if(this!=p){get()->pop_back();return;}
	    if(!tag)tail=tail->pre();
	    else head=head->nxt();
	    if(head==NULL||tail==NULL)head=NULL,tail=NULL;
	}
	void push_front(edge &p){
		...
		if(!tag)head->pre()=&p,p.nxt()=head,head=&p;
		else tail->nxt()=&p,p.pre()=tail,tail=&p;
	    }
	}
	void push_back(edge &p){
		...
		if(!tag)tail->nxt()=&p,p.pre()=tail,tail=&p;
		else head->pre()=&p,p.nxt()=head,head=&p;
	    }
	}
	void reverse(){
...
edge* &edge::pre(){
    if(!col||!col->tag)return _pre;
    else return _nxt;
}
edge* &edge::nxt(){
    if(!col||!col->tag)return _nxt;
    else return _pre;
}
list<chain*>g[maxn];
vector<edge *>ed[maxn];
...
int dfs(int x,chain* pre){
	...
	cout<<"chain "<<v<<' '<<x<<" "<<v->front()->u()<<" "<<v->front()->v()<<" L R "<<v->L<<' '<<v->R<<endl;
...
void modify(int x){
	...
	if((R->pre()!=L))swap(L,R);
	ed[x].resize(0);
	t[++idx].reverse();t[idx].L=x;t[idx].R=c->L;t[idx].front()=L;t[idx].back()=c->front();
	for(auto &v:g[c->L]){
	    if(v==c)v=&t[idx];
	}
	g[x].push_back(&t[idx]);
